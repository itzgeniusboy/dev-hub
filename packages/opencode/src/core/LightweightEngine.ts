...
export class LightweightEngine {
  ...
  static enforceMemoryCap() {
    if (process.memoryUsage().heapUsed > this.MAX_HEAP_MB * 1024 * 1024) {
      console.warn(`[DEV HUB] Memory warning: Exceeded ${this.MAX_HEAP_MB}MB cap. Triggering garbage collection.`);
      if (global.gc) {
        global.gc();
      }
    }
  ...
  static cleanupStaleProcesses(maxAgeMinutes: number = 10) {
    ...
    try {
      // Only clean up processes owned by Dev Hub; never terminate unrelated Termux apps.
      // This requires `ps` and `awk`, which are available in Termux.
      const script = `
        ps -eo pid,etimes,comm | awk '$1 != ${process.pid} && $2 > ${maxAgeMinutes * 60} && $3 ~ /dev-hub|devhub|opencode/ {print $1}' | xargs -r kill -9
      `;
