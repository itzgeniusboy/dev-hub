import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@nexus-ai/client/promise"
...
import { List, type ListRef } from "@nexus-ai/ui/list"
import { ProviderIcon } from "@nexus-ai/ui/provider-icon"
import { Spinner } from "@nexus-ai/ui/spinner"
import { Tag } from "@nexus-ai/ui/tag"
import { TextField } from "@nexus-ai/ui/text-field"
import { ButtonV2 } from "@nexus-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@nexus-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@nexus-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "nexus-go") return language.t("dialog.provider.nexusGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "nexus"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.nexus.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["nexus", "nexus-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "nexus" || provider.id === "nexus-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "nexus"
                                ? "dialog.provider.nexus.tagline"
                                : "dialog.provider.nexusGo.tagline",
                            )}
                          </span>
