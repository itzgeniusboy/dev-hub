...
import { Database } from "@nexus-ai/core/database/database"
import { ProjectTable } from "@nexus-ai/core/project/sql"
import { SessionTable } from "@nexus-ai/core/session/sql"
import { WorkspaceTable } from "@nexus-ai/core/control-plane/workspace.sql"
import { eq } from "drizzle-orm"
import { Hash } from "@nexus-ai/core/util/hash"
import { SessionID } from "@/session/schema"
import { WorkspaceV2 } from "@nexus-ai/core/workspace"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const nexusFile = path.join(tmp, ".git", "nexus")
      expect(yield* Effect.promise(() => Bun.file(nexusFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "nexus")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "nexus")
      const wrongCache = path.join(parentDir, ".git", "nexus")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "nexus")
      const cacheB = path.join(bareB, "nexus")
      const wrongCache = path.join(parentDir, ".git", "nexus")
