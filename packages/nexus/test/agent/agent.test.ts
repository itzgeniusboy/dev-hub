...
it.instance("plan agent denies edits except .nexus/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".nexus/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".nexus", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.NEXUS_TEST_HOME
      process.env.NEXUS_TEST_HOME = test.directory
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          process.env.NEXUS_TEST_HOME = home
        }),
      )
