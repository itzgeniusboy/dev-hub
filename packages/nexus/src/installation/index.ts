...
import { AppProcess } from "@nexus-ai/core/process"
import path from "path"
import { makeRuntime } from "@nexus-ai/core/effect/runtime"
...
export class Service extends Context.Service<Service, Interface>()("@nexus/Installation") {}
  ...
  Effect.gen(function* () {
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        if (process.execPath.includes(path.join(".nexus", "bin"))) return "curl" as Method
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          { name: "brew", command: () => text(["brew", "list", "--formula", "nexus"]) },
          { name: "scoop", command: () => text(["scoop", "list", "nexus"]) },
          { name: "choco", command: () => text(["choco", "list", "--limit-output", "nexus"]) },
        ]
        ...
        for (const check of checks) {
          ...
          const installedName =
            check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "nexus" : "nexus-ai"
      ...
      latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "choco") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27nexus%27%20and%20IsLatestVersion&$select=Version",
        ...
        if (detectedMethod === "scoop") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/nexus.json",
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          case "curl":
            ...
            break
          case "npm":
            upgradeResult = yield* run(["npm", "install", "-g", `nexus-ai@${target}`])
            break
          case "pnpm":
            upgradeResult = yield* run(["pnpm", "install", "-g", `nexus-ai@${target}`])
            break
          case "bun":
          ...
          case "choco":
            upgradeResult = yield* run(["choco", "upgrade", "nexus", `--version=${target}`, "-y"])
            break
          case "scoop":
            upgradeResult = yield* run(["scoop", "install", `nexus@${target}`])
            break
          default:
