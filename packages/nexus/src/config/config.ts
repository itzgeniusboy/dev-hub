...
import { httpClient } from "@nexus-ai/core/effect/app-node-platform"
...
import { InstallationLocal, InstallationVersion } from "@nexus-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@nexus-ai/core/v1/config/console-state"
import { FSUtil } from "@nexus-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@nexus-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@nexus-ai/core/v1/config/config"
import { RemoteAuthError } from "@nexus-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@nexus-ai/core/v1/config/permission"
import { ConfigPluginV1 } from "@nexus-ai/core/v1/config/plugin"
...
export class Service extends Context.Service<Service, Interface>()("@nexus/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://nexus.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.NEXUS_CONFIG && !Flag.NEXUS_CONFIG_DIR && !Flag.NEXUS_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://nexus.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "nexus.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "nexus.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://nexus.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "NEXUS_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/nexus`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://nexus.ai/config.json"
        ...
        if (Flag.NEXUS_CONFIG) {
          yield* merge(Flag.NEXUS_CONFIG, yield* loadFile(Flag.NEXUS_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.NEXUS_CONFIG })
        }

        if (!Flag.NEXUS_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.NEXUS_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from NEXUS_CONFIG_DIR", { path: Flag.NEXUS_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".nexus") || dir === Flag.NEXUS_CONFIG_DIR) {
        ...
        if (process.env.NEXUS_CONFIG_CONTENT) {
          const source = "NEXUS_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.NEXUS_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from NEXUS_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["NEXUS_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("NEXUS_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.NEXUS_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.NEXUS_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("NEXUS_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.NEXUS_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.NEXUS_DISABLE_PRUNE) {
