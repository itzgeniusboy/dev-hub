...
import type { AssistantMessage, Message, NexusClient, SessionMessageResponse } from "@nexus-ai/sdk/v2"
...
export const AuthMethodID = "nexus-login"
...
export class Service extends Context.Service<Service, Interface>()("@nexus/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `nexus auth login` in the terminal",
      name: "Login with nexus",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: NexusClient) {
...
function makeUsageService(sdk: NexusClient) {
...
async function loadDirectorySnapshot(sdk: NexusClient, directory: string) {
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "NEXUS service failure", service })
}
