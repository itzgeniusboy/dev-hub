// Shared type vocabulary for the direct interactive mode (`nexus --mini`).
//
...
import type { NexusClient, PermissionRequest, QuestionRequest, ToolPart } from "@nexus-ai/sdk/v2"
import type { TuiConfig } from "@nexus-ai/tui/config"
...
type PromptModel = Parameters<NexusClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<NexusClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<NexusClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<NexusClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<NexusClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<NexusClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<NexusClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<NexusClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<NexusClient["question"]["reject"]>[0]
