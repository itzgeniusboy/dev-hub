import type { PermissionV1 } from "@nexus-ai/core/v1/permission"
import { FSUtil } from "@nexus-ai/core/fs-util"
// CLI entry point for `nexus run` and `nexus --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`nexus --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`nexus --mini --attach`): connects to a running
//      nexus server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run NEXUS with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running NEXUS server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to NEXUS_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to NEXUS_SERVER_USERNAME or 'nexus')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: NexusClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: NexusClient, sessionID: string) {
      ...
      async function current(sdk: NexusClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: NexusClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: NexusClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: NexusClient) {
        ...
        async function loop(client: NexusClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
