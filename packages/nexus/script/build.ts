...
const createEmbeddedWebUIBundle = async () => {
  ...
  await $`NEXUS_CHANNEL=${Script.channel} bun run --cwd ${appDir} build`
...
for (const item of targets) {
  const name = [pkg.name, targetName(item)].filter(Boolean).join("-")
  ...
  await Bun.build({
    ...
    files: {
      ...
      ...(embeddedFileMap ? { "nexus-web-ui.gen.ts": embeddedFileMap } : {}),
    },
    entrypoints: [
      "./src/index.ts",
      workerPath,
      treeSitterWorkerPath,
      ...(embeddedFileMap ? ["nexus-web-ui.gen.ts"] : []),
    ],
    define: {
      FFF_LIBC: JSON.stringify(item.abi === "musl" ? "musl" : "gnu"),
      NEXUS_VERSION: `'${Script.version}'`,
      NEXUS_MODELS_DEV: generated.modelsData,
      OTUI_TREE_SITTER_WORKER_PATH: bunfsRoot + treeSitterWorkerPath,
      NEXUS_WORKER_PATH: workerPath,
      NEXUS_EMBEDDED_TEMPLATES: JSON.stringify(embeddedTemplates),
      NEXUS_CHANNEL: `'${Script.channel}'`,
      NEXUS_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
